...
На входе подаётся матрица n_channels x n_samples,
где n_channels - число датчиков, n_samples - число измерений. 
...
## Метрики
Рис. 2. Пример подсчёта метрик. a) Сырые данные. b) Полосовой фильтр 10-100Гц 
e) DAS-N2N - нейросетевой подход 

SNR - signal to nosie ratio в bbox вокруг сигнала землетрясения. Достаточно получить метрики больше чем при использовании обычных bandpass фильтров. Например, в исследовании (https://doi.org/10.1093/gji/ggad460)  получены такие соотношения(см. Картинки a, b, e, правый верхний угол)
...
## Датасеты
Данные из эксперимента MiDAS (см. https://g-a51e00.a1bfb5.bd7c.data.globus.org/DAS-Month-02.2023/Academia_Sinica-MiDAS/README.pdf). Содержат записи DAS.

Находятся в репозитории globus.org, в публичной коллекции pubDAS. Имеются директории Events и Continuous. 
...
Ссылка на расположение в репозитории: https://app.globus.org/file-manager?origin_id=706e304c-5def-11ec-9b5c-f9dfb1abb183&origin_path=%2FDAS-Month-02.2023%2FAcademia_Sinica-MiDAS%2F.
## Моделирование
### Бейзлайн
...
### Основная модель
Обучать будем jDAS (https://ieeexplore.ieee.org/document/9655039) 
...
Where n_channels - number of channels, n_samples - number of samples. 
...
SNR - signal to noise ratio in a bounded box around earthquake signal. It’s enough to get metrics more than bandpass filter. For example, in research (https://doi.org/10.1093/gji/ggad460)  gotten these ratios(Fig. 2,  a, b, e, right upper angle)
...
## Data
Data from MiDAS experiment (See https://g-a51e00.a1bfb5.bd7c.data.globus.org/DAS-Month-02.2023/Academia_Sinica-MiDAS/README.pdf). Contains DAS data.

They are located at globus.org, in the public collection pubDAS. There are Events и Continuous directories. 
...
Problems: 1 record volume is 1.37 Gb. It’s needed to processing by parts that usually called patches.

Link to data repository: https://app.globus.org/file-manager?origin_id=706e304c-5def-11ec-9b5c-f9dfb1abb183&origin_path=%2FDAS-Month-02.2023%2FAcademia_Sinica-MiDAS%2F.
...
### Main model
We will train jDAS (https://ieeexplore.ieee.org/document/9655039) 
Fig. 3. jDAS model architecture

Model usually trained by masked channel modelling. I.e. we will take, for example, 11 neighbour channels(sensors), 1 channel randomly is setted to zero and because channels are near we try to reconstruct it. The main idea is that the model will not reconstruct noise, because there is no information about this(because noise is incoherent) but coherent signals may be reconstructed.
